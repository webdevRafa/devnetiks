...
On desktop and mobile, the projects section uses a lazy-loaded 3D desk with the owner-supplied website screenshots as sRGB screen textures. Device clicks and keyboard-accessible project buttons select a project; camera transitions respect reduced motion. Subtle staggered device-ring pulses run only while visible and stop for reduced motion. A WebP capture of the actual desk crossfades into the live scene. The static preview and project controls remain usable if WebGL fails. No-JavaScript visitors retain the original project list. Mobile uses touch-friendly project controls and camera framing that adapts to the canvas aspect ratio. Project headings use the existing brand logos. All site and case-study links remain ordinary HTML links.
